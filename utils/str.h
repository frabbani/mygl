...
class Buffer {
 ...
 public:
  ...
  Buffer(std::string_view str, std::optional<Lut> lut = std::nullopt) {
    init(str, lut);
  }
...
struct LineFeed {
  ...
  struct Visitor {
    ...
    Visitor(std::function<char(void*)> getChar_ = nullptr)
        :
        getChar(getChar_) {
    }
 ...
 public:
  ...
  struct Value {
   public:
    Token name;
    ValueType actual;
    Value(std::string_view nameTag, T value)
        :
        name(nameTag),
        actual(value) {
    }
    Value(std::string_view nameTag, ValueFunc valueFunc)
        :
        name(nameTag),
        actual(valueFunc) {
    }
    Value(TokenFunc nameFunc, ValueFunc valueFunc)
        :
        name(nameFunc),
        actual(valueFunc) {
    }
    Value(TokenFunc nameFunc, T value)
        :
        name(nameFunc),
        actual(value) {
    }
    ...
    bool resolve(T &value, std::string_view token, ValueFunc &from) {
