...
namespace mygl
{
  struct CharSet{
    struct CharMap{
      float x, y, w, h;
      CharMap() :x(0.0f), y(0.0f), w(0.0f), h(0.0f) {}
    };
...
GLboolean MyGL_loadAsciiCharSet( const MyGL_AsciiCharSet *char_set, GLboolean filtered, GLboolean mipmapped  ){

  MyGL_ROImage image = {
      .w = char_set->imageAtlas.w,
      .h = char_set->imageAtlas.h,
      .pixels = char_set->imageAtlas.pixels
  };
  ...
  for( uint32_t i = 0; i < char_set->numChars; i++ ){
    if( char_set->chars[i].c >= MYGL_FIRST_ASCII_PRINTABLE_CHAR &&
        char_set->chars[i].c <= MYGL_LAST_ASCII_PRINTABLE_CHAR ){
