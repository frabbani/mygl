...
/*
    ...
    if( trace::stencilOut.has_value() ){
      ...
      trace::stencilOut.value().print( "mode is %s, ref is %d, test-mask is %x.   ",
                                       stencilModeToString( stencilTest.mode ).data(), stencilTest.ref, stencilTest.mask );
    }
  }
...
/*
  ...
  void debug() {
    ...
    utils::logout( "stencil op: %d, %d, %d", stencilOp.stencilFail, stencilOp.stencilPassDepthFail,
                   stencilOp.stencilPassDepthPass );
  }
...
struct Uniform{
  ...
  Uniform( const Uniform &u ) :
      name( u.name ), type( u.type ), loc( u.loc ), value( u.value ) {
  }
