...
std::optional<shaders::SourceCode::Lines> shaders::SourceCode::expand(const std::string &pass) {
  ...
  for (size_t i = 0; i < source.lines.size(); i++) {
    ...
    if (0 == stack) {
      ...
      if (tokenizer.count >= 2) {
        if (0 == std::strcmp(tokenizer[0].data(), pass.c_str()) && 0 == std::strcmp(tokenizer[1].data(), "{")) {
          stack++;
          continue;
        }
...
shaders::ShaderPass::ShaderPass(const std::string &pass_, const SourceCode::Lines &lines)
    :
    pass(pass_) {
...
//utils::logout( "'%s'", b.kChars() );
  ...
  utils::logout("Shader '%s' Pass '%s' create status: vert=%u  frag=%u  prog=%u", source.c_str(), pass.c_str(), vert, frag, prog);
...
// utils::logout( "vertex shader source length: %d", status );
  ...
  utils::logout("Vertex shader compile status: %s", 1 == status ? "success" : "failed!");
  if ( GL_FALSE == status) {
    Logger logger(s);
  ...
  utils::logout("Fragment shader compile status:  %s", 1 == status ? "success" : "failed!");
  if ( GL_FALSE == status) {
    Logger logger(s);
  ...
  for (GLint i = 0; i < numUniforms; i++) {
    ...
    if (1 == size) {
      ...
      if (MyGL_Debug_getChatty())
        utils::logout(" %d) '%s' size=%d type=%x", i, uniform.name.c_str(), size, type);
